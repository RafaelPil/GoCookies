...
// DPAPI decrypts data using Windows' DPAPI (Data Protection API).
func DPAPI(encryptPass []byte) ([]byte, error) {
	...
	r, _, err := procDecryptData.Call(
		uintptr(unsafe.Pointer(newBlob)),
	...
	copy(d, (*[1 << 30]byte)(unsafe.Pointer(outBlob.pbData))[:outBlob.cbData])
	return d, nil
}

// ASN1PBE defines the interface for ASN.1 password-based encryption decryption.
type ASN1PBE interface {
	Decrypt(globalSalt, masterPwd []byte) (key []byte, err error)
}

// NewASN1PBE creates an appropriate ASN1PBE instance based on the provided byte structure.
func NewASN1PBE(b []byte) (ASN1PBE, error) {
	var (
		n nssPBE
		m metaPBE
		l loginPBE
	...
	return nil, errors.New("failed to decode ASN.1 data")
}

// nssPBE implements the ASN1PBE interface for NSS password-based encryption.
type nssPBE struct {
	AlgoAttr struct {
...
func (n nssPBE) Decrypt(globalSalt, masterPwd []byte) ([]byte, error) {
...
// metaPBE implements the ASN1PBE interface for metadata encryption.
type metaPBE struct {
	AlgoAttr  algoAttr
	Encrypted []byte
}
...
func (m metaPBE) Decrypt(globalSalt, _ []byte) ([]byte, error) {
...
// loginPBE implements the ASN1PBE interface for login-related decryption.
...
func (l loginPBE) Decrypt(globalSalt, _ []byte) ([]byte, error) {
...
// aes128CBCDecrypt decrypts data using AES in CBC mode with PKCS5 padding.
func aes128CBCDecrypt(key, iv, encryptPass []byte) ([]byte, error) {
	...
	if encryptLen < block.BlockSize() {
		return nil, errors.New("encrypted data length is less than block size")
	}
...
// des3Decrypt decrypts data using 3DES in CBC mode with PKCS5 padding.
func des3Decrypt(key, iv, src []byte) ([]byte, error) {
